...
:::{.callout-tip}
这里的AIC，BIC都不是标准模式，是在正态分布线性回归中的一种等效形式。普通线性回归常用`AIC()`函数提取AIC值，时间序列分析中在`ar()`等函数会直接计算AIC值。前者公式是标准公式，后者是现在提供的公式，结果不相同。但AIC具体值不是最重要的，其差值才是我们需要考察的点，所以只需要保证同一个模型迭代中公式一致就行(BIC也一样)。
:::
...
|模型|$k$(df1)|误差平方和(SSE)|自由度(df2)|均方和(MSE)|可决系数($R^2$)|AIC|BIC|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|[@eq-2_18]|`r s1[[1]]$Df[1]`|`r s1[[1]]$"Sum Sq"[2] %>% round(0)`|`r s1[[1]]$Df[2]`|`r s1[[1]]$"Mean Sq" %>% round(1)`|`r summary(fit1)$r.squared %>% round(2)`|`r (AIC(fit1)/num-log(2*pi)) %>% sprintf("%.2f", .)`|`r (BIC(fit1)/num-log(2*pi)) %>% sprintf("%.2f", .)`|
|[@eq-2_19]|`r s2[[1]]$Df[1]`|`r s2[[1]]$"Sum Sq"[2] %>% round(0)`|`r s2[[1]]$Df[2]`|`r s2[[1]]$"Mean Sq" %>% round(1)`|`r summary(fit2)$r.squared %>% round(2)`|`r (AIC(fit2)/num-log(2*pi)) %>% sprintf("%.2f", .)`|`r (BIC(fit2)/num-log(2*pi)) %>% sprintf("%.2f", .)`|
|[@eq-2_20]|`r s3[[1]]$Df[1]`|`r s3[[1]]$"Sum Sq"[2] %>% round(0)`|`r s3[[1]]$Df[2]`|`r s3[[1]]$"Mean Sq" %>% round(1)`|`r summary(fit3)$r.squared %>% round(2)`|`r (AIC(fit3)/num-log(2*pi)) %>% sprintf("%.2f", .)`|`r (BIC(fit3)/num-log(2*pi)) %>% sprintf("%.2f", .)`|
|[@eq-2_21]|`r s4[[1]]$Df[1]`|`r s4[[1]]$"Sum Sq"[2] %>% round(0)`|`r s4[[1]]$Df[2]`|`r s4[[1]]$"Mean Sq" %>% round(1)`|`r summary(fit4)$r.squared %>% round(2)`|`r (AIC(fit4)/num-log(2*pi)) %>% sprintf("%.2f", .)`|`r (BIC(fit4)/num-log(2*pi)) %>% sprintf("%.2f", .)`|
